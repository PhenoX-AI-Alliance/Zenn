...
def convert_to_zenn_queue(draft_path):
    ...
    import re
    # 寄付リンクの強制正規化（エージェントのハルシネーション対策）
    content = re.sub(r'https://ko-fi\.com/[a-zA-Z0-9_-]+', 'https://ko-fi.com/phenox_noc2', content)
    # 本物の決済リンク(cs_live_等を含む)は保護し、偽物のStripeリンクだけをKo-fiに変換する
    content = re.sub(r'https://(?:buy|checkout)\.stripe\.com/(?!.*cs_(live|test)_)[a-zA-Z0-9_/-]+', 'https://ko-fi.com/phenox_noc2', content)
